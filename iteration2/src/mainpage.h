 ...
 * In the first iteration, the robot simulator is more like a video game. A user controls the robot with the arrow keys. The objective of the game is to touch all of the Foods without running into (eventually moving) lights or walls. The robot is given a fixed number of lives at the beginning of the game, but it will lose a life with every collision (except for when colliding with a Food).
 *
 ...
 *
 * The game starts off with the robot and other entities are placed randomly in the window screen. The robot is given 9 lives at the head of the game. The goal is for the robot to capture all of the Foods and avoid lights and walls as much as possible. If the robot touches a wall or an light, it will stop and lose one live. During 2 seconds after the collision, the robot would be granted a protected shield which keeps it invincible to any walls and lights. The invincibility status will be visual to users by color changing of the robot. If the robot touches a Food, it will stop and wait for the next move from user. The Food color also changes to a different color (orange at this time) to notify the user that this Food is already captured. The game will be determined WIN if the robot captures all Foods (3 at this time). The game will be determined LOST if the robot loses all of its lives. When the game is over (WIN or LOST), the user has to hit a "New Game" button to restart.
 *
